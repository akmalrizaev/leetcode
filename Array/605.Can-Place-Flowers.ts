...
function canPlaceFlowers(flowerbed: number[], n: number): boolean {
  ...
  for (let i = 0; i < flowerbed.length; i++) {
    if (flowerbed[i] === 0) {
      // Check the previous and next position (boundary conditions included).
      ...
      if (prevEmpty && nextEmpty) {
        ...
        count++;
        if (count >= n) return true; // Early exit if we planted enough flowers
      }
    }
  }
...
console.log(canPlaceFlowers([0, 0, 1, 0, 1, 0, 0], 1)); // Output: true
